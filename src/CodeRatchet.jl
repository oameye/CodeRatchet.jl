...
struct Row
  ...
  function Row(numbers::Dict{String,Int}, findings::Vector{String}=String[])
    return new(numbers, sort!(copy(findings)))
  end
end
...
# to object identity on its mutable fields, so a row read back from a baseline
# never equals the semantically identical row that was written.
Base.:(==)(a::Row, b::Row) = a.numbers == b.numbers && a.findings == b.findings
Base.hash(row::Row, h::UInt) = hash(row.findings, hash(row.numbers, hash(:Row, h)))
...
"""
    finding_binding(metric) -> Union{Nothing,String}

The numeric binding explained by a finding multiset, or `nothing` for a
count-only metric. Finding-aware adapters must make this number equal the
multiset size in every row.
"""
finding_binding(::Metric) = nothing

"""
    finding_identity(finding) -> String

Canonical location-free identity for one finding. Finding-producing adapters
provide methods for their own diagnostic/report types.
"""
function finding_identity end
...
"""
    measurement_configuration(metric, rulings) -> Dict{String,Any}

Rulings-dependent fields that change what a metric measures. Kept separate
from tool provenance so a non-default ratchet directory cannot accidentally
read configuration from `ratchet_dir(root)`.
"""
measurement_configuration(::Metric, ::Any) = Dict{String,Any}()

"""
    measurement_provenance(metric, root; dir) -> Dict{String,Any}
...
"""
function measurement_provenance(
  metric::Metric, root::AbstractString; dir::AbstractString=ratchet_dir(root)
)
  result = copy(provenance(metric, root))
  for (key, value) in measurement_configuration(metric, read_rulings(dir))
    haskey(result, key) && error("duplicate semantic provenance field $(repr(key))")
    result[key] = value
  end
  get!(result, "julia", string(VERSION.major, ".", VERSION.minor))
...
"""
struct EntryContext{R}
  rows::R
  dir::String
end

function entry_failures(
  metric::Metric, root::AbstractString, paths, rows; dir::AbstractString=ratchet_dir(root)
)
  return entry_failures(metric, root, paths, EntryContext(rows, String(dir)))
end

function entry_failures(metric::Metric, ::AbstractString, paths, context::EntryContext)
  finding_binding(metric) === nothing && return String[]
  bad = String[]
  for path in paths
    n = length(context.rows[path].findings)
    n == 0 || push!(bad, "$path enters with $n reviewed finding(s); new code enters clean")
  end
  return bad
end
...
"""
function ruling_failures(
  metric::Metric, root::AbstractString; dir::AbstractString=ratchet_dir(root)
)
  return ruling_failures(metric, root, dir)
end

ruling_failures(::Metric, ::AbstractString, ::AbstractString) = String[]
...
function read_baseline(metric::Metric, dir::AbstractString)
  ...
  for (file, data) in get(raw, "files", Dict{String,Any}())
    findings = String[String(x) for x in get(data, "findings", String[])]
    numbers = Dict{String,Int}(k => Int(v) for (k, v) in data if k != "findings")
    rows[file] = Row(numbers, findings)
  end
...
function render_baseline(
  ...
  validate_findings(metric, rows, "measurement")
  io = IOBuffer()
  println(
  ...
  for (key, value) in sort(collect(measurement_provenance(metric, root; dir)); by=first)
  ...
  for file in sort(collect(keys(rows)))
    ...
    finding_binding(metric) === nothing ||
      println(io, "findings = ", tomlvalue(row.findings))
  end
...
One semantic finding whose multiplicity increased relative to the baseline.
"""
struct FindingViolation
  path::String
  identity::String
  from::Int
  to::Int
...
function Base.show(io::IO, v::FindingViolation)
  return print(
    io, v.path, ": finding ", repr(v.identity), " multiplicity ", v.from, " -> ", v.to
  )
end
...
function pair_renames(dead, added, baseline, current, keys)
  fingerprint(row) = (Int[get(row, k, -1) for k in keys], row.findings)
...
struct Report
  ...
  finding_violations::Vector{FindingViolation}
...
function validate_findings(metric::Metric, rows::Dict{String,Row}, source::AbstractString)
  key = finding_binding(metric)
  for (path, row) in rows
    if key === nothing
      isempty(row.findings) || error(
        "$source row $path carries findings but $(metric_name(metric)) has no finding binding",
      )
      continue
    end
    haskey(row.numbers, key) ||
      error("$source row $path has no $(repr(key)) count for its finding multiset")
    count = row.numbers[key]
    count == length(row.findings) || error(
      "$source row $path records $key=$count but has $(length(row.findings)) finding(s)"
    )
    issorted(row.findings) || error("$source row $path has an unsorted finding multiset")
  end
  return nothing
end

function finding_multiplicities(findings)
...
function finding_violations(
  metric::Metric,
  current::Dict{String,Row},
  baseline::Dict{String,Row},
  renames::Dict{String,String},
)
  finding_binding(metric) === nothing && return FindingViolation[]
  violations = FindingViolation[]
  for path in sort!(collect(keys(current)))
    oldpath = haskey(baseline, path) ? path : get(renames, path, "")
    isempty(oldpath) && continue
    before = finding_multiplicities(baseline[oldpath].findings)
    now = finding_multiplicities(current[path].findings)
    for identity in sort!(collect(keys(now)))
      from, to = get(before, identity, 0), now[identity]
      to > from && push!(violations, FindingViolation(path, identity, from, to))
    end
  end
  return violations
end
...
function check(
  ...
  validate_findings(metric, current, "current measurement")
  ...
  ruling_bad = ruling_failures(metric, root; dir)
  ...
  provenance_bad = provenance_failures(metric, recorded, root; dir)
  ...
  held = baseline::Dict{String,Row}
  validate_findings(metric, held, "baseline")
  violations, new_files, dead, renames = ratchet(metric, current, held)
  finding_bad = finding_violations(metric, current, held, renames)
  entry = entry_failures(metric, root, new_files, current; dir)
  return Report(
    metric_name(metric),
    violations,
    finding_bad,
...
"""
    refresh_provenance_failures(metric, baseline, recorded, root, dir)

Return provenance incompatibilities for an existing baseline; bootstrap has none.
"""
function refresh_provenance_failures(
  metric::Metric,
  baseline,
  ...
  baseline === nothing && return String[]
  return provenance_failures(metric, recorded, root; dir)
end
...
function refresh(
  ...
  human_rulings = ruling_failures(metric, root; dir)
  ...
  baseline, recorded = read_baseline(metric, dir)
  provenance_bad = refresh_provenance_failures(metric, baseline, recorded, root, dir)
  ...
  if !accept_change && (!isempty(report.violations) || !isempty(report.finding_violations))
    error(refuse_change(metric_name(metric), report.violations, report.finding_violations))
  end
  ...
  write(
    baseline_path(metric, dir),
    render_baseline(metric, measure(metric, root; dir), root; dir),
  )
  return report
end
...
function provenance_failures(
  ...
  expected = measurement_provenance(metric, root; dir)
...
"""
function refuse_change(
  kind::AbstractString,
  violations::Vector{Violation},
  findings::Vector{FindingViolation}=FindingViolation[],
)
  io = IOBuffer()
  for v in violations
    println(io, "ERROR: ", v)
  ...
  n = length(violations) + length(findings)
  print(io, "Re-run with --accept-change to record $n change(s) in $kind.")
  return String(take!(io))
end

function refuse_rise(kind::AbstractString, violations::Vector{Violation})
  return refuse_change(kind, violations)
end
...
function finding_table(violations::Vector{FindingViolation})
  io = IOBuffer()
  println(io, "| file | new finding | baseline multiplicity | now |")
  println(io, "| --- | --- | --- | --- |")
  for v in violations
    identity = replace(v.identity, "|" => "\\|")
    println(io, "| ", v.path, " | `", identity, "` | ", v.from, " | ", v.to, " |")
  end
  return String(take!(io))
end
...
function write_artifact(metric::Metric, root::AbstractString, dir::AbstractString)
  ...
  write(path, render_baseline(metric, measure(metric, root; dir), root; dir))
  return path
end
...
function Base.show(io::IO, report::Report)
  ...
  section("new findings", report.finding_violations)
