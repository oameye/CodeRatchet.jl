function parse_source()
  ...
  isempty(CodeRatchet.parse_failures(pwd(), files)) ||
    error("CodeRatchet source did not parse")
  return length(files)
end

function measure_complexity()
  rows = CodeRatchet.measure(Complexity(), pwd(); dir=joinpath(pwd(), "code_ratchet"))
...
const PRECOMPILE_BENCHMARKS = (
  parse_source=parse_source, measure_complexity=measure_complexity
)
